...
$result4 = mysqli_query($db, "UPDATE `users_1` SET `login` = '$login' WHERE `login` = '$old_login'"); //обновляем в базе логин пользователя 
            ...
            echo "<html><head><meta http-equiv='Refresh' content='5; URL=page.php?id=".$_SESSION['id']."'></head><body>Ваш логин изменен! Вы будете 
            перемещены через 5 сек. Если не хотите ждать, то <a href='page.php?id=".$_SESSION['id']."'>нажмите сюда.</a></body></html>"; }
...
////////////////////////
            ...
            $password = htmlspecialchars($password);$password = trim($password);//удаляем все лишнее 
            if ($password == '') { 
...
$password = md5($password);//шифруем пароль
            ...
            //можно добавить несколько своих символов по вкусу, например, вписав "b3p6f". Если этот пароль будут взламывать методом подбора у 
            // себя на сервере этой же md5,то явно ничего хорошего не выйдет. Но советую ставить другие символы, можно в начале строки или в середине.
            //При этом необходимо увеличить длину поля password в базе. Зашифрованный пароль может получится гораздо большего размера.
 
$result4 = mysqli_query($db, "UPDATE `users_1` SET `password` = '$password' WHERE `login` = '$old_login'");//обновляем пароль 
            ...
            echo "<html><head><meta http-equiv='Refresh' content='5; URL=page.php?id=".$_SESSION['id']."'></head><body>Ваш пароль изменен! Вы будете 
            перемещены через 5 сек. Если не хотите ждать, то <a href='page.php?id=".$_SESSION['id']."'>нажмите сюда.</a></body></html>"; }
...
if (empty($_FILES['fupload']['name']))
            ...
            $result7 = mysqli_query($db, "SELECT `avatar` FROM `users_1` WHERE `login` =  '$old_login'");//извлекаем текущий аватар 
...
$date=time(); //вычисляем время в настоящий момент.
            ...
            // Сделано, чтобы у аватаров не было одинаковых имен.
...
$result7 = mysqli_query($db, "SELECT avatar FROM `users_1` WHERE `login` = '$old_login'");//извлекаем текущий аватар пользователя
...
$result4 = mysqli_query($db, "UPDATE `users_1` SET avatar= '$avatar' WHERE `login` = '$old_login'");//обновляем аватар в базе 
            if ($result4=='TRUE') {//если верно, то отправляем на личную страничку
            echo "<html><head><meta http-equiv='Refresh' content='5; URL=page.php?id=".$_SESSION['id']."'></head><body>Ваша аватарка изменена! 
            Вы будете перемещены через 5 сек. Если не хотите ждать, то <a href='page.php?id=".$_SESSION['id']."'>нажмите сюда.</a></body></html>";}
      } 
?>