...
mysqli_query ($db, "DELETE FROM `oshibka` WHERE UNIX_TIMESTAMP() -    UNIX_TIMESTAMP(date) > 60");
...
//можно добавить несколько своих символов по вкусу, например, вписав "b3p6f". Если этот пароль будут взламывать методом подбора у себя на сервере 
...
$result = mysqli_query($db, "SELECT * FROM `users_1` WHERE `login` = '$login' AND `password` = '$password' AND `activation` = '1'"); 
//мы дописали «AND `activation` = '1'», то есть пользователь будет искаться только среди активированных.
